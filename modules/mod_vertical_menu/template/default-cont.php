...
<nav id="<?php echo $module->instanceid ?>" class="<?php echo $module->instanceid ?> sm-menu <?php echo $params->get('moduleclass_sfx', ''); ?>">
  ...
  <?php if ($params->get('show_modulepos')): ?>
    <div class="sm-modpos <?php echo $params->get('top_module') ?>">
  ...
  <?php if (count($modules = JModuleHelper::getModules($params->get('top_module')))): // TOP MODULEPOS ?>
    <div class="<?php echo $params->get('top_module') ?>">
  ...
  <?php if ($filter[0] && strpos($params->get('position'), 'overlay') === false): ?>
	<div class="sm-filter-cont">
		<input id="sm-filter-<?php echo $module->id ?>" class="sm-filter" type="text" placeholder="<?php echo JText::_($filter[3] ? $filter[3] : 'JSEARCH_FILTER_LABEL') ?>" value="" />
		<label for="sm-filter-<?php echo $module->id ?>" class="sm-search"></label>
