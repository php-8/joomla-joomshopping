		...
		<div class="prod_cart_quan col-md-3">
			...
			<div class="prod_qty_input">
				...
				<?php }else{?>
					...
					<input type = "text" id="quantity" name = "quantity[<?php print $key_id ?>]" value = "<?php print $prod['quantity'] ?>" class = "inputbox" />
					<button class="counter-plus">+</button>
				<?php }?>
                <?php print $prod['_qty_unit']; ?>
				<?php if (!$prod['not_qty_update']){?>
					<div class = "cart_reload">
						<i onclick="document.updateCart.submit();" class="icon-refresh"></i>
					</div>
				<?php }?>
            </div>
			...
			<div id = "checkout">
				...
				<?php if ($countprod>0) : ?>
					<a href = "<?php print $this->href_checkout ?>" class="btn btn-primary">
						<i class="icon-basket"></i> <?php print _JSHOP_CHECKOUT ?>
					</a>
				<?php endif; ?>
				</div>
