...
<form action="<?php print $_SERVER['REQUEST_URI'];?>" method="get" id="mat_js_filter" name="jshop_filters">
    ...
    <?php if ($show_prices){?>
    <div class="filter_price"><div class="filter_price_title"><?php print JText::_('PRICE')?>:</div>

		<div class="range_slider_output">
			<div class="fprice_left_box"><input type = "text" class = "inputbox fprice_from" placeholder="<?php if ($fprice_from>0){print $fprice_from;} else{print 0;}?>" name = "fprice_from" id="fprice_from" size="7" value="<?php if ($fprice_from>0) print $fprice_from?>" /></div>
			<div class="fprice_right_box"><input type = "text" class = "inputbox fprice_to" placeholder="<?php if ($fprice_to>0) print $fprice_to?>" name = "fprice_to"  id="fprice_to" size="7" value="<?php if ($fprice_to>0) print $fprice_to?>" /></div>
		</div>
