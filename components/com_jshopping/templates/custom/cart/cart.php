...
<?php if ($prod['not_qty_update']){?>
                ...
                <?php }else{?>
                    ...
                    <input class="quantity-plus" type="button" value="+" onclick="var qty_el = document.getElementById('quantity[<?php print $key_id ?>]'); var qty = qty_el.value; if( !isNaN( qty )) qty_el.value++; return false;">
                    <input type = "text" id = "quantity[<?php print $key_id ?>]" onkeyup="reloadPrices();" name = "quantity[<?php print $key_id ?>]" value = "<?php print $prod['quantity'] ?>" class = "inputbox" />
                    <input class="quantity-minus" type="button"  value="-" onclick="var qty_el = document.getElementById('quantity[<?php print $key_id ?>]'); var qty = qty_el.value; if( !isNaN( qty ) && qty > 1 ) qty_el.value--; return false;">
