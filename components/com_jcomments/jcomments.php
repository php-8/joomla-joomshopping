...
if (isset($_REQUEST['jtxf'])) {
	...
	$comment_text = preg_replace('/[^A-Za-zА-Яа-я0-9]+/u', "", $comment_text);
	if (preg_match("/(http|https|www|рассылк|арбитраж|юридич|распрод|инет|интернет|объявл|глаук|лазерн|порн|секс|простит|дискус|транзит|мошен|[a-z]+)/i", $comment_text)) {
		exit;
	}
